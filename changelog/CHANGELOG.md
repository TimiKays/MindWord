...
### 云同步优化
1. 新增 Cloudflare Worker 代理，国内访问 Supabase 速度大幅提升；
2. 修复切换代理后登录状态丢失的问题（固定 storageKey + 自动迁移旧 session）；
3. 登录后同步失败不再重试5次，改为提示用户手动重试，减少无效请求；
4. 同步状态查询改为只查时间戳字段，从10秒+降到100ms；
5. 添加同步请求锁，防止并发同步导致数据异常；
6. 移除已弃用的 feedback 表相关代码；
