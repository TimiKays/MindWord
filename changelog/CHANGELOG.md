...
### 统一账户与云同步全面上线
1. 注册登录与云同步全面切换至 TimiKays 统一账户系统
2. 最近2个月未登录的用户，已重置为随机密码，需通过"忘记密码"重置后登录；
3. 云端数据均已迁移完成，重要数据仍建议自行点击左上角导出zip到本地备份；

---
## v1.1.6 (2026/07/14)

### Markdown 支持转图片
1. 在 Markdown 编辑器中新增「转图片」入口，可将当前文档发送到 MarkPage 生成图片卡片；
2. 少量内容可自动传输并直接打开生成页面；
3. 内容过长时可一键复制 Markdown，手动粘贴到 MarkPage 继续使用；
4. 本地图片过大时可能无法自动传输，可手动复制后处理；
...
1. 提升网络访问速度和稳定性；
2. 减少无效云同步请求；
3. 同步状态查询改为只查时间戳字段，提升查询速度，从10秒+降到100ms；
4. 添加同步请求锁，防止重复请求；
5. 用户反馈已改用飞书，清理无效代码；
6. 文件精简；
...
### bug修复
1. 修复左侧编辑器输入时光标自动跳到最后的问题；
2. 左侧编辑器修改内容后，思维导图应保持原来的展开/收起状态，视口尽量不变；
3. 修复节点收起后残留「+」号幽灵按钮的问题；
4. 修复切换颜色主题偶尔失效或反应慢的问题；
5. 修复思维导图编辑后备注莫名添加前置空格的问题；

---

## v1.1.3 (2026/05/06)

1. 每次编辑后的成功提示干扰写作思路，已改为顶部固定图标提示；
2. 大纲视图中支持有序列表序号自增；
3. 线上PWA应用更新太慢，修改缓存机制；
...
### bug修复
1. 防止默认示例数据覆盖真实数据。
2. 修复切换思维导图后层级按钮的布局问题；
3. 在大纲视图中，点击enter后焦点丢失问题；
4. 在大纲视图中，如果当前行没内容，按enter不再新建行；
---

## v1.1.1 (2026/05/04)
### 功能
1. 大纲视图增加拖拽排序功能、备注编辑功能；
2. 登录后的同步预览弹窗更快弹出；
3. word文档预览改为按所选模板样式进行渲染，并提供三种内置word样式模板，更直观；
4. 官网性能优化，减少体积，提升加载速度，增加对大纲视图的介绍；
...
1. 【新增大纲模式】：提供摆脱鼠标的沉浸式文档编辑体验！
2. 【后台架构调整】：为啥这么久没更新呢，因为刚上线就得知原本依赖的平台要倒闭了🥹！拖了很久还是决定做迁移，并继续免费支持现有功能。登录时会自动迁移您的数据，请耐心等待，不影响已有数据和功能；
> （1）如果同步失败，请先导出所有文档到zip备份，然后退出登录后再重新登录，即可继续同步。
> （2）如果忘记了旧平台的登录密码，可先点击重置密码界面最下方的重置旧平台密码按钮后，重置原密码，再登录哦~（此功能为兼容旧平台的临时功能，三个月后会下架）
3. 【反馈方式调整】：改为飞书表单，自带通知，响应更快，之前的反馈已收到，正在处理了；
4. 【点赞与赞赏功能】：本想放红色的心来着，感觉有点干扰，所以改为白色的心啦，干净一些。这是一个 100% 免费的个人项目，如果喜欢的话可以支持一下后续维护哦，一块钱不嫌少，十块钱不嫌多
   (～￣▽￣)～
...
1. 调整工具提示和代码块的字体大小，原字体过小；
2. 修复bug：编辑markdown或者同步数据后，思维导图会退出下钻并无法恢复旧视口的问题；
